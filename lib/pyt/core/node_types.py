...
from lib.pyt.helper_visitors import LabelVisitor

ControlFlowNode = namedtuple(
    "ControlFlowNode", ("test", "last_nodes", "break_statements")
)
...
class Node:
    ...
    def connect(self, successor):
        ...
        if isinstance(self, ConnectToExitNode) and not isinstance(
            successor, EntryOrExitNode
        ):
            return

        self.outgoing.append(successor)
        if successor is not None and successor.ingoing is not None:
...
class RaiseNode(Node, ConnectToExitNode):
    ...
    def __init__(self, ast_node, *, path):
        ...
        super().__init__(label_visitor.result, ast_node, path=path)
    ...
    def __repr__(self):
        ...
        return "".join(
            (
                ...
                "\n",
                "right_hand_side_variables:\t",
                str(self.right_hand_side_variables),
            )
        )
...
class RestoreNode(AssignmentNode):
    ...
    def __init__(
        self, label, left_hand_side, right_hand_side_variables, *, line_number, path
    ):
...
class ReturnNode(AssignmentNode, ConnectToExitNode):
    ...
    def __init__(
        self, label, left_hand_side, ast_node, right_hand_side_variables, *, path
    ):
